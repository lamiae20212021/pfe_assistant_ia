...
def login_page():
    ...
    st.markdown("""
        <style>
            ...
            /* Style du bouton "Créer un compte" pour ressembler à un lien */
            .link-button > button {
    ...
    # ================================
    # LIEN "CRÉER UN COMPTE" CENTRÉ
    # ================================
    st.markdown("<br>", unsafe_allow_html=True)

    colA, colB, colC = st.columns([1, 1, 1])
    with colB:
        create = st.button("Créer un compte", key="create_account_btn", help="Créer un compte", 
                           use_container_width=False, 
                           type="secondary")

    # Appliquer style lien
    st.markdown("""
        <style>
            div[data-testid="stButton"][key="create_account_btn"] > button {
                background: none !important;
                border: none !important;
                color: #00e6ac !important;
                text-decoration: underline !important;
...
def send_email_to_admin(identifiant, email, message):
    try:
        admin_email = "lamiae20212021@gmail.com"
        sender_email = "lamiae20212021@gmail.com"  # obligatoire !
        gmail_password = "sagw suqo oebn guic"  # mot de passe application

        subject = "Demande d'assistance - Réinitialisation mot de passe"
        body = f"""
...
def is_valid_password_signup(pwd):
...
def send_email_verification(email, code):
    try:
        sender_email = "lamiae20212021@gmail.com"   # ton email Gmail
        gmail_password = "sagw suqo oebn guic"      # mot de passe d’application Gmail
...
Veuillez le saisir dans l'application pour finaliser votre inscription.
...
def page_signup():
    ...
    username = st.text_input("Identifiant (veuillez saisir votre prénom)")
    ...
    if st.button("Confirmer mon inscription"):

        # Champs obligatoires
        if username.strip() == "" or email.strip() == "" or pwd.strip() == "":
        ...
        if not is_valid_password_signup(pwd):
            st.error("❌ Mot de passe invalide : une majuscule + un caractère spécial obligatoires.")
            return
        ...
        if not success:
            st.error("❌ Erreur : impossible d'envoyer le code. Vérifiez l'email renseigné.")
            return

        # Aller vers la page de saisie du code
...
def page_signup_verify():
    st.markdown("## ✉️ Vérification de l'email")

    st.info("Un code à 6 chiffres vous a été envoyé par email. Veuillez le saisir ci-dessous.")

    entered = st.text_input("Code reçu")

    if st.button("Créer mon compte"):

        if entered == st.session_state.get("signup_code", ""):
            ...
            with engine.begin() as conn:
                conn.execute(
                    text("""
                        ...
                        VALUES (:u, :p, 'viewer', :e)
                    """),
                    {"u": info["username"], "p": info["password"], "e": info["email"]}
                )

            st.session_state.account_created = True
            st.session_state.page = "signup_success"
            st.rerun()
...
##########################################################
# page succes compte cree
############################################################

def page_signup_success():
    st.success("🎉 Votre compte a été créé avec succès !")

    if st.button("🔙 Revenir à la page de connexion"):
        st.session_state.page = "login"
        st.session_state.account_created = False   # ← indispensable !!!
        st.session_state.signup_code = None
        st.session_state.signup_info = None
        st.rerun()
...
if st.session_state.page == "signup_verify":
    page_signup_verify()
    st.stop()

if st.session_state.page == "signup_success" or st.session_state.get("account_created"):
    st.session_state.page = "signup_success"
    page_signup_success()
    st.stop()
...
def page_reset_request():
    ...
    username = st.text_input(
        "Identifiant",
        value=st.session_state.get("tmp_username", "")
    )
    email = st.text_input("Email")
    ...
    # Ce bouton doit exister en dehors du bloc
    contact_clicked = st.button("📩 Nous contacter")
    ...
    if reset_clicked:
        ...
        if row and row[0] == email:
            ...
            # Activation du bouton Contact Support
            st.session_state.allow_contact = True

    # -------------------------------
    # TRAITEMENT BOUTON NOUS CONTACTER
    # -------------------------------
    if contact_clicked:
        if st.session_state.get("allow_contact", False):
            st.session_state.contact_identifiant = username
            st.session_state.contact_email = email
            st.session_state.page = "contact_support"
            st.rerun()
        else:
            st.warning("Veuillez d'abord renseigner des informations incorrectes pour contacter l'administration.")
...
def contact_support():
    st.markdown("## 📩 Contacter l'administration")

    ident = st.text_input("Identifiant", value=st.session_state.get("contact_identifiant", ""))
    email = st.text_input("Email", value=st.session_state.get("contact_email", ""))

    message = st.text_area(
        "Message",
        placeholder="Décrivez clairement votre problème ici..."
    )

    if st.button("Envoyer"):
        if ident.strip() == "" or email.strip() == "" or message.strip() == "":
            st.error("⚠️ Veuillez remplir tous les champs.")
            return

        # Envoi de l'email
        success = send_email_to_admin(ident, email, message)

        if success:
            st.success("✅ Votre message a été envoyé. L’administrateur vous contactera.")
        else:
            st.error("❌ Une erreur s'est produite lors de l'envoi de votre message.")
...
#########################################
# CARACTERES SPECIAUX MOT DE PASSE 
#########################################
...
# ---------------------------------------
# ROUTING — SIGNUP (pages publiques)
# ---------------------------------------
if st.session_state.page == "signup":
    page_signup()
    st.stop()
...
if st.session_state.page == "signup_success":
    page_signup_success()
    st.stop()

# ---------------------------------------
# ROUTING — RESET REQUEST (public)
# ---------------------------------------
...
# ---------------------------------------
# ROUTING — CONTACT SUPPORT (public)
# ---------------------------------------
if st.session_state.page == "contact_support":
    contact_support()
    st.stop()

# ---------------------------------------
# 🚨 IMPORTANT : LOGIN uniquement APRÈS les pages publiques !
# ---------------------------------------
...
def page_settings():
    ...
    #  ✉️ MODIFIER L'EMAIL
    # ============================
    st.markdown("### ✉️ Modifier l'email")

    colE1, colE2 = st.columns(2)

    with colE1:
        user_email_select = st.selectbox("Utilisateur", user_list, key="email_user")

    # Récupérer l'email actuel depuis la base
    with engine.begin() as conn:
    ...
    with colE2:
        new_email_value = st.text_input(
            "Nouvel email",
            value=current_email if current_email else ""
        )

    update_email_btn = st.button("Modifier l'email")

    if update_email_btn:
        if new_email_value.strip() == "":
            st.session_state["email_msg"] = ("error", "⚠️ L'email ne peut pas être vide.")
        else:
            try:
                with engine.begin() as conn:
                    conn.execute(
                        text("UPDATE users SET email = :e WHERE username = :u"),
                        {"u": user_email_select, "e": new_email_value}
                    )
                st.session_state["email_msg"] = (
                    "success",
                    f"Email de **{user_email_select}** mis à jour."
                )
            except Exception as e:
                st.session_state["email_msg"] = ("error", f"Erreur SQL : {e}")

        st.rerun()

    # ⭐ MESSAGE AFFICHÉ ICI
    if "email_msg" in st.session_state:
        msg_type, msg = st.session_state["email_msg"]
        if msg_type == "success":
            st.success(msg)
        else:
            st.error(msg)
        del st.session_state["email_msg"]
