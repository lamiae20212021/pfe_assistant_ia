...
def send_email_to_admin(identifiant, email, message):
    try:
        ...
        body = f"""
Un utilisateur rencontre un problème pour réinitialiser son mot de passe :
...
{message}
        ...
        # SMTP Gmail — utiliser un mot de passe d'application
        smtp_server = "smtp.gmail.com"
        smtp_port = 587
        gmail_user = admin_email
        gmail_password = "sagw suqo oebn guic"  # <<< IMPORTANT !!!
...
def send_email_verification(email, code):
    try:
        sender_email = "lamiae20212021@gmail.com"
        subject = "Votre code de vérification"
        body = f"""
Bonjour,

Votre code de vérification pour créer votre compte est : {code}

Veuillez le saisir dans l'application pour confirmer votre inscription.
...
def page_signup():
    ...
    if st.button("Confirmer mon inscription"):
        ...
        if not success:
            st.error("❌ Erreur : impossible d'envoyer le code. Vérifiez l'email.")
            return
...
def page_signup_verify():
    ...
    st.info("Un code à 6 chiffres vous a été envoyé. Veuillez le saisir ci-dessous.")
